...
The market loop is deterministic from a seed. Each round creates a market regime with a trend, volatility, liquidity condition, and occasional Solstice event (Sun Flare, Thin Books, or Reversal Watch); generates a Binary, Caesar, or logic signal; lets the player preview the actual bid/ask before committing a spread; simulates fills and price movement; then marks cash plus inventory to market for PnL. The sunset debrief adds a PnL-by-round chart, best matched decision, and largest inventory-risk moment.
